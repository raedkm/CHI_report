...
\begin{abstract}
\noindent
This document defines the epidemiological methods underpinning the Community Health Indicators (CHI) reporting system. The system is organized into two methodologically distinct modules built on a common patient-month analytical grain across four chronic conditions: Diabetes Mellitus (DM), Hypertension (HTN), Dyslipidemia (DLP), and Obesity (OB).

\begin{itemize}[leftmargin=*, itemsep=2pt]
    \item \textbf{Module 1 --- Epidemiological Surveillance} answers \textit{how much disease exists and how fast is it appearing?} using three report types: monthly screening coverage, annual point prevalence, and monthly incidence. Base population: \textbf{at-risk} cohort (undiagnosed). Thresholds: \textit{screening} cut-points.
    \item \textbf{Module 2 --- Compliance \& Care Gap} answers \textit{of those with disease, how well are we managing them and closing follow-up gaps?} using three report types: annual disease control classification, quarterly follow-up completion, and annual care-gap distribution. Base population: \textbf{prevalent} cohort (diagnosed). Thresholds: \textit{control} targets.
\end{itemize}
...
The \textbf{patient-month}. Each eligible patient contributes up to 12 monthly observations. The patient-month is the analytical grain from which all six epidemiological measures --- screening, prevalence, incidence (Module~1) and control, quarterly care gap, annual care gap (Module~2) --- are derived.
...
Cluster stratification is applied identically in Module 2 (Reports 4--6); see §7 for report-specific rate forms.
...
\begin{table}[ht]
\centering\small
\caption{Summary of all rate formulas (6 reports across 2 modules)}
