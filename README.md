...
| No dynamic at-risk population — the cohort was static | Patients were either "diabetic" or "not diabetic" based on a pre-identified cohort list, not ICD-10 codes. At-risk status never changed month-to-month. | `stg_{cond}_patient_month` evaluates `is_at_risk_start` at each month boundary — a patient diagnosed in March leaves the at-risk pool for April onward |
| Incidence was not measurable | Without first-ever diagnosis dates from ICD-10 codes, you can't distinguish a new case from a pre-existing one | `stg_{cond}_patient_month` compares `first_e11_date` against each month's date range to flag `is_incident_case` |
| Lab screening classification was absent | The legacy query counted tests done but didn't categorize results (normal / elevated / abnormal) | The analytical view applies clinical thresholds (e.g., ACC/AHA for HTN, WHO for BMI, dual-unit FBS for DM) and assigns a `screening_category` |

**Concrete example:** The legacy `archive/NMR_queries/Diabitic report 14-6-26.sql` (175 lines) was built against a pre-existing cohort of known diabetic patients — it assumed the population was already identified, so it couldn't answer "who is at risk this month?" or "is this a new case?" The staging system starts from raw EMR tables (`NMR.LEANHIS.PATIENTS`, `DIAGNOSIS_CODES`, `LABRESULTS`, `OBSERVATIONS`) and derives cohort membership, at-risk status, screening classification, and incident case detection from ICD-10 diagnoses and lab results — making all three report types possible from the same source data.
